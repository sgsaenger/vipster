...
sed "s|X_PYROOT_X|${PY_PREFIX}|" "${SOURCE_DIR}/util/select_python_hook.sh" | sed "s|X_PYBIN_X|${PY_BIN##*/}|" > AppDir/util/select_python_hook.sh
...
chmod +x linuxdeploy*
OUTPUT=Vipster-Linux-x86_64.AppImage ./linuxdeploy-x86_64.AppImage --appdir AppDir -o appimage -p qt -p checkrt
