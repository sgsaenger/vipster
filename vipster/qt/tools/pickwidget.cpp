...
void PickWidget::updateWidget(guiChange_t change)
{
    ...
    for(size_t i=0; i<nat; ++i){
        text.appendPlainText(QString::number(idx[i].first)+'('+
    ...
    if(nat>1){
        ...
        printDist(text, idx[0].first, idx[1].first, dist01);
        if(nat>2){
            ...
            printDist(text, idx[0].first, idx[2].first, dist02);
            printDist(text, idx[1].first, idx[2].first, dist12);
            if(nat == 3){
                ...
                auto ang201 = (std::acos(Vec_dot(diff01, diff02) / (dist01 * dist02)))
                              * rad2deg;
                printAngle(text, idx[0].first, idx[1].first, idx[2].first, ang012);
                printAngle(text, idx[1].first, idx[2].first, idx[0].first, ang120);
                printAngle(text, idx[2].first, idx[0].first, idx[1].first, ang201);
            }else{
                ...
                printDist(text, idx[0].first, idx[3].first, dist03);
                printDist(text, idx[1].first, idx[3].first, dist13);
                printDist(text, idx[2].first, idx[3].first, dist23);
                ...
                auto dihed2013 = (std::acos(Vec_dot(cross012, cross013) / (len012 * len013)))
                                 * rad2deg;
                printDihed(text, idx[0].first, idx[1].first, idx[2].first, idx[3].first, dihed0123);
                printDihed(text, idx[0].first, idx[1].first, idx[3].first, idx[2].first, dihed0132);
                printDihed(text, idx[1].first, idx[0].first, idx[2].first, idx[3].first, dihed1023);
                printDihed(text, idx[1].first, idx[2].first, idx[3].first, idx[0].first, dihed1230);
                printDihed(text, idx[1].first, idx[0].first, idx[3].first, idx[2].first, dihed1032);
                printDihed(text, idx[2].first, idx[0].first, idx[1].first, idx[3].first, dihed2013);
            }
        }
    }
}
