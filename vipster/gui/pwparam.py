...
class PWParam(QTreeWidget):
    ...
    def createParameter(self):
        ...
        if 'New' in self.pw[str(parent.text(0))]:
            return
    ...
    def deleteItem(self):
        if self.currentItem().parent():
            del self.pw[str(self.currentItem().parent()
                        .text(0))][str(self.currentItem().text(0))]
    ...
    def itemHandler(self, item, column):
        ...
        # parameter value
        if column:
            self.pw[str(item.parent().text(0))][str(item.text(0))] =\
                str(item.text(1))
        # parameter
        elif item.parent():
            ...
            self.pw[str(item.parent().text(0))] =\
                OrderedDict([(str(item.text(0)), v) if k == old[0]
                            ...
                            self.pw[str(item.parent().text(0))].items()])
