...
class Picker(QTextEdit):
    ...
    def setMol(self, mol):
        ...
        if sel:
            ...
            if len(sel) > 1:
                diff01 = at[0][1] + dot(sel[0][1], vec) - at[1][1] -\
                    dot(sel[1][1], vec)
                output += u'Dist {1}-{2}: {0:3.3f} Å\n'.format(
                    norm(diff01), *ids[:2])
            if len(sel) > 2:
                diff12 = at[1][1] + dot(sel[1][1], vec) - at[2][1] -\
                    dot(sel[2][1], vec)
                output += u'Dist {1}-{2}: {0:3.3f} Å\n'.format(
                    norm(diff12), *ids[1:3])
                if len(sel) > 3:
                    diff23 = at[2][1] + dot(sel[2][1], vec) - at[3][1] -\
                        dot(sel[3][1], vec)
                    output += u'Dist {1}-{2}: {0:3.3f} Å\n'.format(
                        norm(diff23), *ids[2:])
                if norm(diff01) > 0 and norm(diff12) > 0:
                    a012 = degrees(arccos(dot(diff01, -diff12) /
                                   (norm(diff01) * norm(diff12))))
            ...
            if len(sel) > 3:
                if norm(diff12) > 0 and norm(diff23) > 0:
                    a123 = degrees(arccos(dot(diff12, -diff23) /
                                   (norm(diff12) * norm(diff23))))
                    output += u'Angle {1}-{2}-{3}: {0:3.3f}°\n'.format(
                        a123, *ids[1:])
                else:
                    output += u'Angle {1}-{2}-{3}: {0}°\n'.format(
                ...
                if norm(c012) > 0 or norm(c123) > 0:
                    d0123 = degrees(arccos(dot(c012, c123) / (norm(c012) *
                                    norm(c123))))
