...
from vipster.iowrapper import availParam, newParam, _guiInNames, _guiOutNames,\
    _paramdict, readFile, writeFile
...
class MainWindow(QMainWindow):
    ...
    def initEditMenu(self):
        ...
        def copyFun():
            ...
            self.copyBuf = []
            for i in sel:
                self.copyBuf.append(deepcopy(
                    self.mol.getAtom(i, charge=True, fix=True, fmt='bohr')))
    ...
    def initFinish(self):
        ...
        if g:
            self.restoreGeometry(
                g if isinstance(g, QByteArray) else g.toByteArray())
        if g:
            self.restoreState(
                w if isinstance(w, QByteArray) else w.toByteArray())
...
# I/O FUNCTIONS #
    ...
    def loadMol(self):
        ...
        if not fname:
            return
        ftype = QInputDialog.getItem(self, "Choose File type", "File type:",
                                     list(_guiInNames.keys()), 0, False)
        if not ftype[1]:
            return
    ...
    def saveMol(self):
        ...
        if not fname:
            return
        ftype = QInputDialog.getItem(self, "Choose File type", "File type:",
                                     list(_guiOutNames.keys()), 0, False)
        if not ftype[1]:
            return
