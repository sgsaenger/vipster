...
class MolTable(collapsibleWidget):
    ...
    def fillTable(self):
        ...
        self.table.setVerticalHeaderLabels(
            [str(x) for x in range(self.mol.nat)])
...
class MolCell(collapsibleWidget):
    def __init__(self, parent):
        ...
        self.updatedisable = False
        super(MolCell, self).__init__('Cell Geometry', self.container,
                                      show=False)
        ...
        def cdmHandler():
            if self.updatedisable:
                return
            if float(self.cellDm.text()) == self.mol.getCellDim():
                return
            self.mol.initUndo()
            if self.appAll.isChecked():
                self.mol.setCellDimAll(
                    float(self.cellDm.text()), self.scale.isChecked())
            else:
                self.mol.setCellDim(
                    float(self.cellDm.text()), self.scale.isChecked())
    ...
    def setMol(self, mol):
        if self.sender() in [self.vtable, self.cellDm]:
            return
        self.mol = mol
...
class MolKPoints(collapsibleWidget):
    def __init__(self, parent):
        ...
        self.container.showEvent = showEvent
        self.updatedisable = False
        super(MolKPoints, self).__init__(
            'K-Points', self.container, show=False)
        ...
        self.disc.table.setHorizontalHeaderLabels(
            ['x', 'y', 'z', 'weight/count'])
        ...
        def cellHandler():
            ...
            for i in range(self.disc.table.rowCount()):
                disc.append([str(self.disc.table.item(i, j).text())
                             for j in range(4)])
        ...
        def newKpoint():
            ...
            for i in range(4):
                self.disc.table.setItem(
                    self.disc.table.rowCount() - 1, i, QTableWidgetItem('1'))
            self.updatedisable = False
            cellHandler()
