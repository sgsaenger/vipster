...
py::class_<S> bind_step(py::handle &m, std::string name){
    ...
    // BONDS
        .def("getBonds", [](const S& s, float c, BondPolicy l, BondFrequency u){
            ...
            }, "cutfac"_a=settings.bondCutFac.val, "level"_a=settings.bondPolicy.val, "update"_a=settings.bondFreq.val)
        .def("setBonds", &S::setBonds,
             "level"_a=settings.bondPolicy.val, "cutfac"_a=settings.bondCutFac.val)
    // SELECTION
